...
| 7 | turn == "complete" かつ翌夜の準備が未了 | 翌夜のタスク案を出力（タスク設計基準 + Row 7 出力形式に従う）。parse_output.py が commander/proposals/ にファイルを書き出す。人間の承認後に .nightly/ へ反映される |
...
Row 7 でタスク案を出力する場合、上記の ACTION/RESULT ブロックに加えて、
以下の形式で proposal ファイルの内容を出力すること。
parse_output.py がこのブロックを検出し、commander/proposals/ 配下にファイルとして書き出す。

```
PROPOSAL_FILE: commander/proposals/YYYY-MM-DD/tasks.yml
<<<CONTENT
（ここに tasks.yml の内容）
>>>END_PROPOSAL

PROPOSAL_FILE: commander/proposals/YYYY-MM-DD/T1-01.md
<<<CONTENT
（ここに T1-01 プロンプトの内容）
>>>END_PROPOSAL

PROPOSAL_FILE: commander/proposals/YYYY-MM-DD/T2-01.md
<<<CONTENT
（ここに T2-01 プロンプトの内容）
>>>END_PROPOSAL
```
...
注意:
- パスは必ず `commander/proposals/` で始めること（それ以外は拒否される）
- YYYY-MM-DD は翌夜の night 日付
- ファイル内容は `<<<CONTENT` と `>>>END_PROPOSAL` の間に記述する
- 1回の出力に複数の PROPOSAL_FILE ブロックを含めてよい

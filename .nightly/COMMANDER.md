...
| 3 | pending_reviews が非空 | 最初のPR番号をレビュー（下記「レビュー手順」参照） |
| 4 | turn == "turn1" かつ turn1完了条件を満たす | turn-switch を実行（gh workflow run） |
| 5 | turn == "turn2" かつ turn2完了条件を満たす | integration → main マージを実行 |
| 6 | turn == "complete" かつ翌夜の準備が未了 | 翌夜の tasks.yml を設計（タスク設計基準に従う） |
| 7 | pending_tasks が非空 | session-watchdog で進捗確認 |
| 8 | 上記いずれにも該当しない | 何もしない。"no-action-needed" をログに記録して終了 |
...
5. **protected_paths 配下のファイル変更**: `.github/`, `AGENTS.md`、および `.nightly/` 配下（ただし、司令塔自身が判断表 Row 6「翌夜のタスク設計」を実行する際の `.nightly/tasks.yml` および `.nightly/prompts/` への書き込みは除く。この禁止は Jules の PR による変更を対象とする）

...
| 7 | turn == "complete" かつ翌夜の準備が未了 | 翌夜の tasks.yml を設計（タスク設計基準に従う） |
...
5. **protected_paths 配下のファイル変更**: `.github/`, `AGENTS.md`、および `.nightly/` 配下（ただし、司令塔自身が判断表 Row 7「翌夜のタスク設計」を実行する際の `.nightly/tasks.yml` および `.nightly/prompts/` への書き込みは除く。この禁止は Jules の PR による変更を対象とする）

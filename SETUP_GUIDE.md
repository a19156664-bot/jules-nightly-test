...
6. **PRの自動公開は `automationMode: AUTO_CREATE_PR` に依存**します。Julesはコード変更完了時にPRを自動公開するため、人間が Jules UI で「Publish PR」を押す運用は不要です。ただしセッションが `FAILED` や承認/フィードバック待ちで停滞するとPRは出ません。この取りこぼしは `session-watchdog` ワークフロー(夜間30分おき)がセッション状態を記録し、翌朝レポートに `未提出(セッション: FAILED)` のように表示して可視化します(監視のみで自動介入はしません)。
